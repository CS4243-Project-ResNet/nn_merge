...
class PoseMLP(nn.Module):
    def __init__(self, in_mlp, out_mlp):
        ...
        self.mlp = nn.Sequential(
            ...
            nn.ELU(),
            nn.Linear(2*in_mlp, math.floor(in_mlp/2)),
            nn.ELU(),
...
class MergeMLP(nn.Module):
    def __init__(self, pose_mlp, seg_resnet, n_in, n_out=3):
        ...
        self.stack_mlp = nn.Sequential(
            ...
            nn.ELU(),
            nn.Linear(math.floor(self.stack_in_dim/2), 2*n_out),
            nn.ELU(),
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val', 'test']:
            ...
            if phase == 'val' and epoch_acc > best_acc:
                best_acc = epoch_acc
                best_model_wts = copy.deepcopy(model.state_dict())
                torch.save(best_model_wts, "runs/merge_mlp_03.pt")
...
def print_classification_report(model_ft):
    ...
    outputs = model_ft(poses, imgs, weapons)
    _, preds = torch.max(outputs, 1)
...
if __name__ == "__main__":
    ...
    my_resnet = ResNet(BasicBlock, [5, 5, 5], 6)
    my_pose_mlp = PoseMLP(201, 4)
    my_stack = MergeMLP(my_pose_mlp, my_resnet, 11)
