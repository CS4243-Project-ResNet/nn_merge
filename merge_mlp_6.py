...
class ResPoseMLP(nn.Module):
    def __init__(self, in_mlp, out_mlp):
        ...
        self.mlp = nn.Sequential(
            ...
            nn.Linear(math.floor(in_mlp/2), math.floor(in_mlp/4)),
            nn.ELU(),
            nn.Linear(math.floor(in_mlp/4), 4*out_mlp),
            nn.ELU(),
            nn.Linear(4*out_mlp, 2*out_mlp),
            nn.ELU()
        )
        self.shortcut = nn.Sequential()
        self.dim_l = nn.Linear(2*out_mlp, 2*out_mlp)
        self.dim = nn.Sequential(
            nn.Linear(2*out_mlp, 2*out_mlp),
            nn.ELU()
        )
        self.elu = nn.ELU()
        self.final = nn.Linear(2*out_mlp, out_mlp)
...
class MergeMLP(nn.Module):
    def __init__(self, pose_mlp, seg_resnet, n_pose_in, n_img_in, n_out=3):
        super(MergeMLP, self).__init__()
        self.pose_mlp = pose_mlp
        self.seg_resnet = seg_resnet
        self.n_pose_in = n_pose_in
        self.n_img_in = n_img_in
        self.stack_in_dim = n_pose_in + n_img_in + 1 # pose_mlp.modules[-1].out_features + seg_resnet.modules[-1].out_features + 1
        self.n_out = n_out
        self.stack_mlp = nn.Sequential(
            nn.Linear(self.stack_in_dim, math.floor(self.stack_in_dim/2)),
            nn.ELU(),
            nn.Linear(math.floor(self.stack_in_dim/2), self.stack_in_dim),
            nn.ELU()
        )
        self.pose_layer = nn.Linear(self.stack_in_dim, self.n_pose_in)
        self.img_layer = nn.Linear(self.n_pose_in, self.n_img_in)
        self.final_layer = nn.Linear(self.n_img_in, self.n_out)
        self.shortcut = nn.Sequential()
        self.elu = nn.ELU()
        self.softmax = nn.Softmax(dim=1)

    def pose_shortcut(self, x, pose_o):
        x1 = self.pose_layer(x)
        x1 += self.shortcut(pose_o)
        x1 = self.elu(x1)
        return x1
    
    def img_shortcut(self, x, img_o):
        x1 = self.img_layer(x)
        x1 += self.shortcut(img_o)
        x1 = self.elu(x1)
        return x1
    ...
    def forward(self, pose, masked, weap):
        ...
        x6 = self.pose_shortcut(x5, pose)
        # x6 = self.img_layer(x6)
        # x6 = self.elu(x6)
        x7 = self.img_shortcut(x6, masked)
        x7 = self.final_layer(x7)
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val', 'test']:
            ...
            if phase == 'val' and epoch_acc > best_acc:
                best_acc = epoch_acc
                best_model_wts = copy.deepcopy(model.state_dict())
                torch.save(best_model_wts, "runs/merge_mlp_06_2.pt")
...
if __name__ == "__main__":
    ...
    my_stack = MergeMLP(my_pose_mlp, my_resnet, 3, 3)
